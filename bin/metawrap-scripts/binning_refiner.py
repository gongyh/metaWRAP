...
parser.add_argument("-1", required=True, help="first bin folder name")

parser.add_argument("-2", required=True, help="second bin folder name")

parser.add_argument("-3", required=False, help="third bin folder name")

parser.add_argument("-o", required=True, help="output folder name")
...
if args["3"] == None:
    print(
        (
            "Specified 2 input bin sets: -1 %s -2 %s"
    ...
    print(
        (
            "Specified 3 input bin sets: -1 %s -2 %s -3 %s"
...
for bin_folder in input_bin_folder_list:
    ...
    bin_folder_bins = [
        os.path.basename(file_name) for file_name in glob.glob(bins_files)
    ]
    ...
    if len(bin_folder_bins) == 0:
        print(
            (
                "No input bin detected from %s folder, please double-check!"
                % (bin_folder)
            )
        )
        exit()
    ...
    if len(bin_folder_bins_ext_list_uniq) > 1:
        print(
            (
                "Different file extensions were found from %s bins, please use same extension (fa, fas or fasta) "
...
for each_folder in input_bin_folder_list:
    ...
    for each_bin in each_folder_bins:
        ...
        new = open(
            "%s/%s/%s_new/%s_%s.fasta"
            % (wd, output_folder, each_folder, each_folder, bin_file_name),
            "w",
        )
    ...
    os.system(
        "cat %s/%s/%s_new/*.fasta > %s/%s/combined_%s_bins.fa"
...
if len(input_bin_folder_list) == 2:
    ...
    os.system(
        "cat %s %s > %s"
        % (
            pwd_combined_folder_1_bins,
            pwd_combined_folder_2_bins,
            combined_all_bins_file,
        )
    )
...
contig_assignments_file_sorted_one_line = (
    "%s/%s/contig_assignments_sorted_one_line.txt" % (wd, output_folder)
)
os.system(
    "cat %s | sort > %s" % (contig_assignments_file, contig_assignments_file_sorted)
)
...
for each in contig_assignments_sorted:
    ...
    if current_match == "":
        ...
        if current_length_total >= bin_size_cutoff:
            contig_assignments_sorted_one_line.write(
                "Refined_%s\t%s\t%sbp\t%s\n"
                % (
                    n,
                    current_match,
                    current_length_total,
...
for each_refined_bin in refined_bins:
    ...
    if len(input_bin_folder_list) == 2:
        ...
        separated_1_handle.write(
            "%s\t%sbp\t%s\n"
            % (
                each_refined_bin_name,
                each_refined_bin_length,
                "\t".join(each_refined_bin_source),
            )
        )
        separated_2_handle.write(
    ...
    while m < len(each_refined_bin_source) - 1:
        googlevis_input_handle.write(
            "%s,%s,%s\n"
            % (
                each_refined_bin_source[m],
                each_refined_bin_source[m + 1],
                each_refined_bin_length_mbp,
            )
        )
        m += 1
