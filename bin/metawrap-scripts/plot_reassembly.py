...
for file_name in sys.argv[4:]:
    ...
    for line in open(file_name):
        ...
        if int(line.split("\t")[5]) > max_n50:
            max_n50 = int(line.split("\t")[5])
for bin_set in data:
...
# Make sure your axis ticks are large enough to be easily read.
plt.yticks(
    list(range(min_completion, 105, 10)),
...
for key, value in sorted(
    iter(ranks.items()), key=lambda k_v: (k_v[1], k_v[0]), reverse=True
):
    rank_order[key] = n
    n += 1
...
# Make sure your axis ticks are large enough to be easily read.
plt.yticks(
    list(range(0, max_contamination + 1, 2)),
...
# save figure
print(
    "Saving figures reassembly_results.eps and reassembly_results.png to folder "
    + sys.argv[1]
)
