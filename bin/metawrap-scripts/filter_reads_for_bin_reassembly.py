...
print(
    "Parsing sam file and writing reads to appropriate files depending what bin they alligned to..."
)
files = {}
opened_bins = {}
for line in sys.stdin:
    ...
    if bin_name not in opened_bins:
        ...
        files[sys.argv[2] + "/" + bin_name + ".strict_1.fastq"] = open(
            sys.argv[2] + "/" + bin_name + ".strict_1.fastq", "w"
        )
        files[sys.argv[2] + "/" + bin_name + ".strict_2.fastq"] = open(
            sys.argv[2] + "/" + bin_name + ".strict_2.fastq", "w"
        )
        files[sys.argv[2] + "/" + bin_name + ".permissive_1.fastq"] = open(
            sys.argv[2] + "/" + bin_name + ".permissive_1.fastq", "w"
        )
        files[sys.argv[2] + "/" + bin_name + ".permissive_2.fastq"] = open(
            sys.argv[2] + "/" + bin_name + ".permissive_2.fastq", "w"
        )
    ...
    if cumulative_mismatches < strict_snp_cutoff:
        files[sys.argv[2] + "/" + bin_name + ".strict_1.fastq"].write(
            "@" + F_cut[0] + "/1" + "\n" + F_cut[9] + "\n+\n" + F_cut[10] + "\n"
        )
        files[sys.argv[2] + "/" + bin_name + ".strict_2.fastq"].write(
            "@" + R_cut[0] + "/2" + "\n" + R_cut[9] + "\n+\n" + R_cut[10] + "\n"
        )

    # permissive assembly
    if cumulative_mismatches < permissive_snp_cutoff:
        files[sys.argv[2] + "/" + bin_name + ".permissive_1.fastq"].write(
            "@" + F_cut[0] + "/1" + "\n" + F_cut[9] + "\n+\n" + F_cut[10] + "\n"
        )
        files[sys.argv[2] + "/" + bin_name + ".permissive_2.fastq"].write(
            "@" + R_cut[0] + "/2" + "\n" + R_cut[9] + "\n+\n" + R_cut[10] + "\n"
        )
