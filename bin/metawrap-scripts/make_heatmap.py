...
def set_colors_to_timeline(df):
    ...
    for sample in df.columns.values:
        if "2013-04" in sample:
            lut.append("m")
        elif "2014-09" in sample:
            lut.append("r")
        elif "2015-06" in sample:
            lut.append("b")
        elif "2015-12" in sample:
            lut.append("c")
        elif "2016-02" in sample:
            lut.append("y")
        elif "2017-02" in sample:
            lut.append("g")
        else:
            lut.append("w")
    return lut
...
def draw_clustermap(df, lut):
    ...
    if lut != False:
        ...
        g = sns.clustermap(
            df, figsize=(14, 8), col_cluster=True, yticklabels=True, cmap="magma"
        )
...
if len(sys.argv) < 3:
    out = ".".join(sys.argv[1].split(".")[:-1]) + ".png"
else:
    out = sys.argv[2]
